...
export default function ContactForm() {
  ...
  const buildWhatsAppMessage = () => {
    const lines = [
      "*Plise Perde — İletişim Formu*",
      "",
      `*Ad Soyad:* ${form.name}`,
    ];
    if (form.company) lines.push(`*Firma/Şirket:* ${form.company}`);
    lines.push(
      `*Telefon:* ${form.phone}`,
      `*E-Posta:* ${form.email}`,
      `*Konu:* ${form.subject || "Belirtilmedi"}`,
      "",
      `*Mesaj:*`,
      form.message,
    );
    ...
    return (
      <div className="p-6 bg-white border border-brand-border rounded-xl">
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <h3 className="text-base font-semibold text-brand-text mb-2">Mesajınız Gönderildi</h3>
          <p className="text-sm text-brand-text-light mb-6">En kısa sürede size dönüş yapacağız.</p>
  ...
  return (
    <div className="p-6 bg-white border border-brand-border rounded-xl shadow-sm">
      <div className="mb-5">
        <h2 className="text-lg font-semibold text-brand-text mb-1">İletişim Formu</h2>
